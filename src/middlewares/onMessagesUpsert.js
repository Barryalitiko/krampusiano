...
function renderMessages() {
  ...
  const filtered = filterVal ? messages.filter(m => m.sender.includes(filterVal)) : messages;
  ...
  for (const msg of filtered) {
    ...
    const audioHtml = msg.audio ? `<audio controls><source src="${escapeHtml(msg.audio)}" type="audio/mp3"></audio>` : "";
    let mediaHtml = "";
    if (msg.imageUrl) {
