...
fsp.mkdir(SERVICES_DIR, { recursive: true }).then(() => {
  console.log("📁 Carpeta 'services' asegurada.");
});
...
function renderMessages() {
  ...
  for (const msg of filtered) {
    ...
    if (msg.imageUrl) {
      mediaHtml += `
        <img class="thumb" src="${escapeHtml(msg.imageUrl)}" onclick="showModal('img', '${escapeHtml(msg.imageUrl)}')" alt="Imagen recibida" />
      `;
    }
    if (msg.videoUrl) {
      mediaHtml += `
        <img class="thumb" src="/services/gallery/video-thumb.png" onclick="showModal('video', '${escapeHtml(msg.videoUrl)}')" alt="Video recibido" />
      `;
    }
    div.innerHTML = `
...
if (msg.videoMessage) {
  ...
  videoUrl = `/services/gallery/${vidFilename}`;
    ...
    const data = {
      ...
      sender: senderJid,
      chat: remoteJid,
      timestamp: webMessage.messageTimestamp,
    };
